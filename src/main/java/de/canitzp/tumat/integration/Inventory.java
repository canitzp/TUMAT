...
public class Inventory implements IWorldRenderer {
    ...
    @Override
    public TooltipComponent renderTileEntity(WorldClient world, EntityPlayerSP player, TileEntity tileEntity, EnumFacing side, TooltipComponent component, boolean shouldCalculate) {   if (tileEntity.hasCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side)) {
        IItemHandler handler = tileEntity.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, side);
            if ((handler.getSlots() > 0 && handler.getSlots() < 1001) || (player.isSneaking() && handler.getSlots() > 0)) {
                ...
                for (int i = 0; i < handler.getSlots(); i++) {
                    if (handler.getStackInSlot(i) == null) { //TODO change or add in 1.11 to ItemStack.NULL_STACK
                        freeSlots++;
                    }
                }
